...
void Visualizer::VisualizeSurroundingLaneNet(
  ...
  for (auto iter = lane_net.lane_set.begin(); iter != lane_net.lane_set.end();
    ...
    // Safety check: skip if lane_points is empty
    if (iter->second.lane_points.empty()) {
      continue;
    }
...
void Visualizer::VisualizeLocalLanes(
  ...
  // Safety: limit the number of lanes to visualize to prevent excessive computation
  const int max_lanes = 100;
  int lane_count = 0;
  
  for (const auto &p_lane : local_lanes) {
    if (lane_count >= max_lanes) break;
    ...
    // Additional safety check: ensure lane is valid
    if (!p_lane.second.IsValid()) {
      continue;
    }

    visualization_msgs::Marker mk;
    // GetMarkerByLane already has safety checks inside, so we can call it directly
    // If it fails, it will return kIllegalInput and we skip that marker
    if (common::VisualizationUtil::GetMarkerByLane(
        p_lane.second, 1.0, Vec3f(1.0, 0.0, 0.0),
        common::cmap.at("magenta").set_a(0.2), -0.3, &mk) == kSuccess) {
  ...
  for (const auto &p_traj : openloop_pred_trajs) {
    ...
    for (const auto &ps : p_traj.second) {
      ...
      common::VisualizationUtil::GetRosMarkerCylinderUsingPoint(
          common::Point(pt), Vec3f(0.5, 0.5, 0.1), common::cmap.at("yellow"),
          0, &point_marker);
      mks.markers.push_back(point_marker);
    }
    visualization_msgs::Marker line_marker;
    common::VisualizationUtil::GetRosMarkerLineStripUsingPoints(
        points, Vec3f(0.1, 0.1, 0.1), common::cmap.at("yellow"), 0,
        &line_marker);
