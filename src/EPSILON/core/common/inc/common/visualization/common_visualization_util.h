...
class VisualizationUtil {
 public:
  ...
  static ErrorType GetMarkerBySpline(const Spline<N_DEG, N_DIM>& spline,
                                     ...
                                     visualization_msgs::Marker* marker) {
    // Safety check: ensure spline has valid domain (check before accessing)
    // First check if domain is valid
    auto vec_domain = spline.vec_domain();
    if (vec_domain.empty() || vec_domain.size() < 2) {
      return kIllegalInput;  // Invalid spline
    }
    
    // Check num_segments matches domain (poly_.size() should be vec_domain.size() - 1)
    int num_segs = spline.num_segments();
    int expected_segs = static_cast<int>(vec_domain.size()) - 1;
    if (num_segs == 0 || num_segs != expected_segs || expected_segs < 1) {
      return kIllegalInput;  // No polynomial segments or mismatch
    }
    
    // Additional check: ensure domain values are valid
    for (size_t i = 0; i < vec_domain.size(); ++i) {
      if (!std::isfinite(vec_domain[i])) {
        return kIllegalInput;  // Invalid domain value
      }
    }
    
    decimal_t s_begin = spline.begin();
    decimal_t s_end = spline.end();
    if (s_begin >= s_end || !std::isfinite(s_begin) || !std::isfinite(s_end)) {
      return kIllegalInput;  // Invalid domain range
    }
    ...
    // Limit the number of points to avoid excessive computation
    const int max_points = 1000;
    int point_count = 0;
    
    // Use a safe step size to avoid infinite loops
    decimal_t safe_step = std::max(step, (s_end - s_begin) / max_points);
    
    // Additional safety: ensure we don't evaluate outside the valid domain
    // Clamp s to be within [s_begin, s_end] to avoid accessing poly_[-1] or poly_[num_segs]
    // Use a small epsilon to avoid evaluating exactly at s_end
    decimal_t s_epsilon = std::max(1e-6, (s_end - s_begin) * 1e-6);
    decimal_t s_max = s_end - s_epsilon;
    
    for (decimal_t s = s_begin; s < s_max && point_count < max_points; s += safe_step) {
      // Clamp s to valid range to prevent out-of-bounds access in evaluate
      decimal_t s_clamped = std::max(s_begin, std::min(s, s_max));
      
      Vecf<N_DIM> ret;
      ErrorType eval_result = spline.evaluate(s_clamped, 0, &ret);
      if (eval_result == kSuccess) {
      ...
      // If evaluation fails, continue to next point (don't break)
    }
