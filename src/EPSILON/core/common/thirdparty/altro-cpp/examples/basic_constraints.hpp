...
/**
 * @brief Waypoint constraint - constrains position (x, y) at a specific knot point
 * 
 * This is useful for forcing the trajectory through intermediate waypoints.
 */
class WaypointConstraint : public constraints::Constraint<constraints::Equality> {
 public:
  WaypointConstraint(double x_target, double y_target) 
      : x_target_(x_target), y_target_(y_target) {}

  static constraints::ConstraintPtr<constraints::Equality> Create(double x, double y) {
    return std::make_shared<WaypointConstraint>(x, y);
  }

  std::string GetLabel() const override { return "Waypoint Constraint"; }
  int StateDimension() const override { return 6; }  // Assumes 6-state vehicle model
  int OutputDimension() const override { return 2; }  // x and y position

  void Evaluate(const VectorXdRef& x, const VectorXdRef& u, Eigen::Ref<VectorXd> c) override {
    ALTRO_UNUSED(u);
    c(0) = x(0) - x_target_;  // x position error
    c(1) = x(1) - y_target_;  // y position error
  }
