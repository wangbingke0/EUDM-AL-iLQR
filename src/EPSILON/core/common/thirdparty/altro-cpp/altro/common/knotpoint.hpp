 ...
 public:
  ...
  std::string ToString(int width = 9) const {
    ...
    for (int i = 0; i < this->n_; ++i) {
      out << std::setw(width) << std::setprecision(3) << State()(i) << " ";
    }
    out << "] u: [";
    for (int i = 0; i < this->m_; ++i) {
      out << std::setw(width) << std::setprecision(3) << Control()(i) << " ";
    }
    out << "] t=" << std::setprecision(2) << GetTime() 
        << ", h=" << std::setprecision(2) << GetStep();
    return out.str();
  }
