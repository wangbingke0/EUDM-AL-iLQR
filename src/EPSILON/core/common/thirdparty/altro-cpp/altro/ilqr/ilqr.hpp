 ...
 public:
  ...
  void InitializeFromProblem(const problem::Problem& prob) {
    ALTRO_ASSERT(prob.NumSegments() == N_,
                 "Number of segments in problem should be equal to the number of segments in the solver");
  ...
  void Solve() {
    ...
    ALTRO_ASSERT(Z_->NumSegments() == N_,
                 "Initial trajectory must have correct length");
