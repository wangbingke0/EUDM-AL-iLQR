...
ErrorType EudmManager::Run(
  ...
  // * I : Prepare
  // 更新 ego_lane_id_、last_snapshot_ 等；
  // 计算这帧要给 EudmPlanner 的参考速度 ref_vel；
  // 根据上一帧 winner 的当前执行进度，生成 desired_action，调用 bp_.UpdateDcpTree(desired_action)，固定本帧 DCP 树的第 1 层动作。
  ...
  // * II : RunOnce
  // 取出当前 DcpTree 的 27 条动作序列；
  // 对每条序列做多层前向仿真（自车 + 周车）；
  // 每层调用 CostFunction + RSS + 严格碰撞检查；
  // 汇总得到 winner_id_、winner_score_ 等。
  ...
  // * III: Summarize
  // SaveSnapshot：把这次规划的所有中间结果打包进 snapshot：
  // 每条序列的 progress_cost、tail_cost、forward_trajs、risky_res 等。
  // ReselectByContext：在原始 winner 基础上，结合 lane change context / task 约束 再做一层筛选：
  // 例如避免频繁触发 lane change、控制 active lane change 的触发时机等；
  // 得到最终用于执行/输出的 processed_winner_id（可能与原 winner 不同）。
  Snapshot snapshot;
  ...
  // 用最终 winner 序列的行为，调用 GetRefLaneForStateByBehavior 拟合这帧的参考车道 ref_lane，供后端跟踪/控制用。
  // 把 snapshot 存到 last_snapshot_，供下一帧的 Prepare / active lane change 模块使用。
  // GenerateLaneChangeProposal：根据这次 winner 的特征，多帧一致性等，更新 last_lc_proposal_，决定是否在若干秒后“推荐”一次 lane change。
  // 更新 context_（重规划上下文）：记录本帧 winner 的动作序列和起始时间，为下一轮 Prepare 提供“当前正在执行什么”的依据。
